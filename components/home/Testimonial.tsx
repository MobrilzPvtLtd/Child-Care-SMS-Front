...
const testimonials: Testimonial[] = [
  ...
  {
    name: 'Alyssa D.',
    title: 'Executive Director at McNeilly Center for Children in Nashville, TN',
...
export default function TestimonialsCarousel() {
  ...
  const visibleTestimonials = testimonials.slice(current, current + 3).concat(
    testimonials.slice(0, Math.max(0, current + 3 - testimonials.length))
  );

  return (
    <section className="bg-gray-100 py-12 px-4 text-center mb-4">
      <h2 className="text-2xl md:text-3xl font-bold mb-1">What educators are saying</h2>
      <p className="text-sm text-gray-600 mb-8">
        Read more testimonials in{' '}
        <a href="#" className="text-indigo-600 underline">customer stories</a>
      </p>
